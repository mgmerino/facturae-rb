...
| **Invoice** | `invoice_number is required`, `invoice_number must not be 'unset'`, `invoice_header contains unknown key: <key>`, `issue_data contains unknown key: <key>`, `totals contains unknown key: <key>` + nested `taxes_output[i].*`, `taxes_withheld[i].*`, `invoice_lines[i].*` |
...
| **FacturaeDocument** | `invoices must not be empty`, `seller and buyer must have different tax_id_number` + nested `invoices[i].*`, `file_header.*`, `seller_party.*`, `buyer_party.*` |
