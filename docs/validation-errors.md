...
| **Subject** | `type must be :individual or :legal`, `name_field1 must be a String`, `name_field2 is required for individuals`, `name_field2 must be a String` + nested `address_in_spain.*`, `overseas_address.*` |
| **Party** | `person_type_code must be F or J`, `residence_type_code must be R, E, or U`, `tax_id_number must be a String`, `tax_id_number is not a valid NIF/NIE` (Spanish natural persons), `tax_id_number is not a valid CIF` (Spanish legal entities) + nested `subject.*` |
